...
def calc1(files_dir):
    var = files_dir
    rang_ir = pd.read_csv(var+"/rang_ir.csv")
    ...
    def calc_razr(list_S_baz, years, ir_num):
        ...
        if len(years) > 1:
            for i in range(1, len(years)):
                print('S_nak {:d} Год {:s} S_baz {:.0f} S_nak(i-1) {:.0f} I(i-1) {:.2f}'.format(ir_num + 1, years[i],
                                                                                                list_S_baz[years[i]],
                                                                                                S_nak[i - 1],
                                                                                                return_k_years(
                                                                                                    years[i - 1])))
    ...
    def calc_priob(priob_nach, year, ir_num):
        print('{:d} ИР приобрет = {:d} * {:.5f} * {:.2f}'.format(ir_num + 1, priob_nach, return_I_g(year),
                                                                 (1 - ((type_ir['текущий'].iloc[ir_num] - 1)
                                                                       /
                                                                       (type_ir['планируемый'].iloc[ir_num])))))
    ...
    test = [0, 0, 0, 0, 0, (4875862.0+5970590.0)/2, 0, 6235400.0, 10697987.0, 0] #смотрим что было снизу при выводе, если не нравится Д.А. то правим на то что тут
    print(test)
